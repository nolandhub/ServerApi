...
import { getStatusColor } from '@/helper/getStatusTicket'
            ...
            <PopoverContent className='w-96 max-h-[80vh] overflow-y-auto' align="start" sideOffset={4}>
                <div className='space-y-2'>
                    ...
                    <div className='space-y-3'>
                        ...
                        {/* Pick up - THÊM OPTIONAL CHAINING */}
                        {data.pickUp && (
                            <div className='flex gap-2'>
                        ...
                        {/* Drop off - THÊM OPTIONAL CHAINING */}
                        {data.dropOff && (
                            <div className='flex gap-2'>
                    ...
                    <div className='space-y-2'>
                        <div className='flex items-center justify-between'>
                            ...
                            <div className='flex items-center gap-1 text-xs text-muted-foreground'>
                                ...
                                <span>{data.totalPassCount || 0} ghế</span>
                            </div>
                        </div>
                            ...
                            {data.option?.map((opt, idx) => (
                                <div key={idx} className='flex items-center justify-between p-2 bg-muted/50 rounded-md'>
                                    ...
                                    <div className='text-right'>
                                        <p className='text-sm font-medium'>{formatCurrency(opt.subtotal || 0)}</p>
                                        <p className='text-xs text-muted-foreground'>x{opt.quantity || 0}</p>
                                    </div>
                                </div>
                            ))}
                            <div className='flex items-center justify-end space-x-1.5 bg-muted/50 rounded-md p-2'>
                                <p className='font-semibold'>Tổng tiền:</p>
                                <p className='text-lg font-bold text-blue-700'>{formatCurrency(data.total || 0)}</p>
                            </div>
                        </div>
                    </div>
